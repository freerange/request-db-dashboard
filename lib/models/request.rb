class Request < ActiveRecord::Base
  ...
  def self.average_response_rate(source = Source.last, response_status = nil)
    ...
    query = query.where("completed_lines.status = #{response_status}") if response_status
    res = query.first.average_request_time
    (res * 1000).round.to_f / 1000
  end

  def self.slow(source = Source.last, response_status = nil)
